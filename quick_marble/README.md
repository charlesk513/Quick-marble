...
## Open decision before coding starts
Should quotation numbers (`QM0001`) be a single global counter across all 4 offices, or should each office have its own independent sequence (e.g. still `QM0001` format but counted separately per office, which could cause duplicate numbers across offices)? I've assumed **one global counter** above to guarantee uniqueness — flag if you want per-office sequences instead.
